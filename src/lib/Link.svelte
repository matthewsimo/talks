<script lang="ts">
	import { anchorClass, focusClass } from './classes';

	export let classes = ``;
	export let href = ``;

	const allClasses = classes ? `${focusClass} ${classes}` : `${focusClass} ${anchorClass}`;
...
<a class={`${allClasses} group`} {href} target={isInternal ? '_self' : `_blank`}>
	...
	{#if !isInternal}
		<span class="hidden w-5 align-middle group-hover:inline-block">
