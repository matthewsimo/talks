<script lang="ts">
	...
	const menuIndicatorClass = `menu-indicator transition-all duration-300 ease-in-out-back opacity-0 translate-x-full -translate-y-4 block absolute rounded-sm h-2 w-1/3 -top-6 bg-main-primary`;
	...
	<div class="flex flex-row justify-between">
		...
		<ul class={`${menuClass} ${justClicked && '[&_>_span]:bg-main-700'}`}>
			...
			<li class={`${menuItemClass} ${isPage('/labs') ? currentClass : ''}`}>
				<a on:click={onClick} class={headerAnchorClass} href="/labs">Labs</a>
			</li>
